  ...
  scenario('creates a message', async (scenario: StandardScenario) => {
    const result = await createMessage({
      input: {
        ...
        recipientId: scenario.message.two.recipientId,
        content: 'String',
        updatedAt: '2026-04-02T19:08:10.260Z',
      },
    })

    expect(result.senderId).toEqual(scenario.message.two.senderId)
    expect(result.recipientId).toEqual(scenario.message.two.recipientId)
    expect(result.content).toEqual('String')
    expect(result.updatedAt).toEqual(new Date('2026-04-02T19:08:10.260Z'))
  })
